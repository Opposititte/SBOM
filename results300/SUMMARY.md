...
## バージョン一致（strict）— 216リポジトリ合算の TP/FP/FN

照合をより厳しく「パス**かつ**バージョン一致」にした場合の合計値（216リポジトリの総和）。

| ツール | TP | FP | FN | macro-F1 | micro-P | micro-R | micro-F1 |
|--------|----:|----:|----:|---------:|--------:|--------:|---------:|
| syft            | 11866 | 2325 | 12180 | 73.7 | 83.6 | 49.3 | 62.1 |
| trivy           | 10894 | 1977 | 13152 | 73.6 | 84.6 | 45.3 | 59.0 |
| cdxgen          |  6569 |  517 | 17477 | 54.6 | 92.7 | 27.3 | 42.2 |
| cyclonedx-gomod |  8780 |   23 | 15266 | 52.9 | 99.7 | 36.5 | 53.5 |

- **TP** = パスもバージョンも一致 / **FP** = 報告したが正解に無い（または版違い）/ **FN** = 見逃し（または版違い）。
- **cyclonedx-gomod は FP がわずか23件**（micro-P 99.7%）— Go ツールチェイン経由でルートモジュールのみを
  解析するため、誤検出と版ずれがほぼ無い。
- 名前一致との差（F1 の低下）は小さく、各ツールとも**検出できた依存のバージョンはほぼ正確**。
- per-repo の strict な TP/FP/FN は `metrics.csv` の `tp_ver` / `fp_ver` / `fn_ver` 列にあります。
