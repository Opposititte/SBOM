...
`_manifest.csv` の実測。**注意: manifest は 2733 行あるが、コンテナリセット→再開で
clone 失敗 10 repo が 2〜3 回記録され重複しているため、ユニーク repo は 2723**（スライドの収集総数と一致）。
**件数は「ユニーク repo」で数えるのが正しい**（生の行数で数えると重複 10 件だけ多く出る）:

| 区分 | ユニーク件数 |
|---|---:|
| クローン総数（ユニーク） | 2723 |
| `gt_go_list.txt` が空（go list が空を返した） | 518 |
| `gt_go_list.txt` が無い（clone 失敗の 10 repo） | 10 |
| **非Go＝go list 空（空 518 ＋ 無し 10）** | **528** ✅ スライドと一致 |
| `go list -m all` 成功（非空） | 2195 |

> 補足: 生の manifest 行で `go_list==fail` を数えると 518、`else(非ok)` で数えると重複込み 538 になる。
> **正は 528（ユニーク）。** 538 は clone 失敗 10 repo の重複行を二重計上した数で、誤り。

この 528 のうち空 gt の 518 件を `errors.txt` の実際のエラー文で分類すると、
**「非Go」は大半正しいが完全ではない**:

| 失敗理由 | 件数 | 割合 | 実際に Go か |
|---|---:|---:|---|
| `go.mod file not found`（モジュールでない） | 458 | 88% | **非Go / 非モジュール**（正しく除外） |
...
| （別枠）clone 失敗 = go list 空の「無し 10」 | 10 | — | 取得不可 |

※ 上表の 458/49/11 は「空 gt 518 件」の内訳。clone 失敗 10 件を足して 528。
