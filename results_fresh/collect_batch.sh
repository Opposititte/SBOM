...
set -u
ROOT=/home/user/SBOM
OUT="$ROOT/results_fresh"; DATA="$OUT/data"; WORK=/tmp/fresh_work
CSV="$OUT/metrics_fresh.csv"
...
KEEP_RAW=30
TO=300
HDR="repo,tool,n_all_tp,n_all_fp,n_all_fn,n_imp_tp,n_imp_fp,n_imp_fn,n_impT_tp,n_impT_fp,n_impT_fn,v_all_tp,v_all_fp,v_all_fn,v_imp_tp,v_imp_fp,v_imp_fn,v_impT_tp,v_impT_fp,v_impT_fn,fp_test,fp_otherOS,fp_direct_unused,fp_indirect_unused,fp_gosum_only"
[ -f "$CSV" ] || echo "$HDR" > "$CSV"
...
commit_push() {
  ...
  git -C "$ROOT" -c user.name='Opposititte' -c user.email='201308377+Opposititte@users.noreply.github.com' \
    commit -q -m "fresh re-collection progress: $1 repos in CSV" >/dev/null 2>&1
...
while IFS=, read -r name url rest; do
  ...
  # メトリクス計算 → CSV
  node "$OUT/metric_one.js" "$outdir" >> "$CSV" 2>/dev/null && { ok=$((ok+1)); DONE_MAP[$name]=1; }
  rm -rf "$d"
