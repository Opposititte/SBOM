...
### ⚠️ 重要な留保：95repo版の代表性に疑い（要・全repo確定）

全repo再収集の途中経過（ランダム順 ~90 repo）では **cdxgen 兄弟 ~31%（indirect未使用 ~47%）** と出ており、
retain-95 の **兄弟77%** と大きく食い違う。retain-95 は gossamer/spire 等のネストモジュール持ち大型repoに
偏っていた可能性が高い。**よって §2/§2d に記録した「cdxgen ネスト兄弟 ~75%」は代表性が未確認で、
確定値ではない。** 全repo（ランダム順）再収集の結果が出るまで暫定扱いとする。

### 全repo（1489）での兄弟分離 — 再収集中（ランダム順）
