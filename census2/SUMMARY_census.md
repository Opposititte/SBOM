...
`go list -m all` は外部依存だけでなく**対象プロジェクト自身のモジュールも出力する**。
GT-imported / GT-impT は生成コマンドのパイプ内で `grep -v "^${gmain} \?$"` により自モジュールを落としているが、
**GT-all の生成行にはこの grep が無い**（`proc.sh:65`）。したがって:
